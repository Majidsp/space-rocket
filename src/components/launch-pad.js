...
function Header({ launchPad }) {
    ...
    const isInFavoriteLaunchPads = favorites.favoriteLaunchPads.find(
        (item) => item.id === launchPad.id
    );
    const dispatch = useDispatch();

    const handleFavoriteLaunchPad = (launchPad) => {
        if (!isInFavoriteLaunchPads) {
            dispatch(addToFavoriteLaunchPads(launchPad));
        } else {
